...
export let aptosClient = new Aptos(
  new AptosConfig({ network: currentNetwork.network }),
);
...
export function calculateAptosAddressFromPublicKey(
  ...
  try {
    ...
    if (publicKeyBytes.length !== 65) {
      throw new Error(
        `Incorrect public key length: ${publicKeyBytes.length}, should be 65 bytes`,
      );
    }
    ...
    if (publicKeyBytes[0] !== 0x04) {
      throw new Error(
        `Incorrect public key format: first byte should be 0x04, actual is 0x${publicKeyBytes[0].toString(16)}`,
      );
    }
...
export function parsePublicKey(response: PublicKeyCredential): Uint8Array {
  ...
  // Convert from COSE
  const publicKey = convertCOSEtoPKCS(
...
/**
 * Credential payload safe to persist with JSON (no Uint8Array — it does not round-trip).
 */
 ...
 */
export function parseStoredCredentialInfo(json: string): CredentialInfo | null {
  try {
    ...
    return {
      id,
      type,
      publicKey: { base64, hex, aptosAddress },
    };
  } catch {
    return null;
  }
...
export function normalizeS(
  ...
  const sig = p256.Signature.fromBytes(
    sigBytes,
    formFormat as ECDSASignatureFormat,
  );
...
export async function submitTransfer(
  ...
  try {
    ...
    const finalSenderAddress =
      senderAddress || credentialData.publicKey.aptosAddress;
    ...
    const simpleTxn = await aptosClient.transaction.build.simple({
      ...
      data: {
        ...
        functionArguments: [finalReceiverAddress, finalAmount],
        typeArguments: [],
      },
      options: {
        maxGasAmount: 2000,
    ...
    console.log(
      "clientDataJSON",
      Buffer.from(clientDataJSON).toString("utf-8"),
    );
    console.log(
      "authenticatorData",
      Buffer.from(authenticatorData).toString("utf-8"),
    );
    console.log("signature", Buffer.from(signature).toString("utf-8"));

    const signatureCompact = normalizeS(
    ...
    console.log(
      ...
      transactionAuthenticator.bcsToHex().toString(),
    );
 ...
 */
export async function checkTransactionStatusWithTimeout(
  transactionHash: string,
): Promise<string> {
